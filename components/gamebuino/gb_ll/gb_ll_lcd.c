...
{
    // BUGFIX : l'ancien test ne verifiait que le decalage total dans le
    // buffer (x + y*SCREEN_WIDTH < SCREEN_WIDTH*SCREEN_HEIGHT), pas x et y
    // separement. Un pixel hors-ecran a droite (x >= SCREEN_WIDTH) passait
    // quand meme ce test et s'ecrivait au debut de la ligne suivante
    // (ex: x=325,y=100 -> offset 32325 -> ligne 101, colonne 5) au lieu
    // d'etre ignore -> artefact visuel "le bord droit reapparait a gauche,
    // une ligne plus bas".
